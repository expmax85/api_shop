...
from users.forms import RegisterForm, \
    VerificationForm, RestorePasswordForm
from users.services import get_auth_user, \
    get_user_and_change_password, get_user
...
def otp_token(request: HttpRequest) -> 'redirect':
    """
    Token getting
    """
    user = get_user(email=request.user.email)
    if user.is_verified:
        redirect('users-polls:status')
    device = user.twiliosmsdevice_set.get()
    device.generate_challenge()
    return redirect(reverse('users-polls:verify'))
